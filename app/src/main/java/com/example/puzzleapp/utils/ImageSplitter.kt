    ...
    for (row in 0 until cr) {
        ...
        for (col in 0 until cr) {
            ...
            val piece = PuzzlePiece(
                context,
                (pieceWidth + offsetX).toFloat(),
                (pieceHeight + offsetY).toFloat()
            )

            piece.correctPoint =
                PointF(
                    (xCoord - offsetX + imageView.left).toFloat(),
                    (yCoord - offsetY + imageView.top).toFloat()
                )
